...
特点：
- 专为韩中漫画翻译优化的 Prompt
- 通过 Ollama 远程调用（默认 localhost:11434，可通过 CUSTOM_OPENAI_API_BASE 配置）
- 无降级链，失败即报错
...
class Qwen3KoZhTranslator(CustomOpenAiTranslator):
    ...
    # 降低 max_tokens: 父类默认 4096 -> num_predict=2048，文本量大的页面
    # 会触发模型生成满 2048 token，导致推理时间暴增(83s+)和 GPU 满载。
    # 改为 2048 -> num_predict=1024，足够覆盖漫画翻译场景。
    _MAX_TOKENS = 2048
    ...
    def __init__(self, model=None, api_base=None, api_key=None):
        ...
        # 默认 Ollama 地址
        api_base = api_base or os.environ.get('CUSTOM_OPENAI_API_BASE', 'http://localhost:11434/v1')
    ...
    async def _request_translation(self, to_lang: str, prompt: str) -> str:
        ...
        _prompt_eval = data.get('prompt_eval_count', 0)

        self.logger.info(
            f'[Qwen3KoZh TIMING] build={_t1-_t0:.2f}s '
            f'http={_t2-_t1:.2f}s parse={_t3-_t2:.3f}s '
            f'total={_t3-_t0:.2f}s '
            f'prompt_len={_prompt_len} eval={_eval_count} prompt_eval={_prompt_eval}'
        )
