...
Usage: python test/benchmark.py [页数默认5]
"""
...
async def main():
    ...
    dest_dir = image_dir.parent / "c13-bench"
    ...
    model_load = first_page - other_avg if n_images > 1 else 0
    print(f"  首次加载: {first_page:.1f}s (含det/ocr/inpaint模型)")
    print(f"  后续每页: {other_avg:.1f}s")
    if n_images > 1:
        print(f"  模型加载: ~{model_load:.1f}s")
    ...
    if n_images >= 2:
        print(f"\n  {'稳定性分析':─^65}")
        pg = page_times[1:]  # exclude first (has model load)
        avg_p = sum(pg) / len(pg)
        mn = min(pg)
        mx = max(pg)
    ...
    # 预估
    avg = sum(page_times[1:]) / (n_images - 1) if n_images > 1 else page_times[0]
    print(f"\n  {'推测翻译时间':─^65}")
    scenarios = [
        ("第13话(157页)", 157),
        ("第13话(每页)", 1),
        ("第13-14话(约11页)", 11),
        ("第13-23话(约11话×40页)", 440),
    ]
    for label, pages in scenarios:
        est = avg * pages + (model_load if pages > 0 else 0)
        h = int(est // 3600)
        m = int((est % 3600) // 60)
        s = est % 60
        print(f"  {label:<28} ~{est/60:.1f} 分钟 ({h}h{m}m{s:.0f}s)")
